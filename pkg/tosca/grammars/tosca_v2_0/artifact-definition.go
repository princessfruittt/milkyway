...
type ArtifactDefinition struct {
	*Entity `name:"artifact definition"`
	...
	ArtifactVersion   *string `read:"artifact_version" yaml:"artifact_version,omitempty"` // introduced in TOSCA 1.3
	Properties        Values  `read:"properties,Value" yaml:"properties,omitempty"`       // ERRATUM: ommited in TOSCA 1.0-1.2 (appears in artifact type)
	RepositoryName    *string `read:"repository" yaml:"repository,omitempty"`
	File              *string `read:"file" yaml:"file,omitempty"` // required only if cannot be inherited
	DeployPath        *string `read:"deploy_path" yaml:"deploy_path,omitempty"`
	ChecksumAlgorithm *string `read:"checksum_algorithm" yaml:"checksum_algorithm,omitempty"` // introduced in TOSCA 1.3
	Checksum          *string `read:"checksum" yaml:"checksum,omitempty"`                     // introduced in TOSCA 1.3
